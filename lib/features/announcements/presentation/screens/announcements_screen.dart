...
class _AnnouncementsScreenState extends State<AnnouncementsScreen> {
  ...
  Future<void> _handlePublish() async {
    ...
    try {
      final payload = {
        ...
        'priority': _priority,
        'announcement_type': _priority,
        'is_active': true,
      };

      await _client.from('system_announcements').insert(payload);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('📢 تم بث الإعلان لجميع المستهدفين في المنظومة بنجاح!'),
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = _announcements.where((a) {
      ...
      if (_selectedFilter == 'للأطباء 🩺' && role != 'DOCTOR') return false;
      if (_selectedFilter == 'للصيدليات 💊' && role != 'PHARMACY') return false;
        ...
        children: [
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      Text('الفئة المستهدفة: ', style: GoogleFonts.cairo(fontSize: 13, fontWeight: FontWeight.bold, color: AdminColors.textPrimary)),
                      const SizedBox(width: 10),
                      Expanded(
                        child: Wrap(
                          spacing: 8,
                      ...
                      ElevatedButton.icon(
                        ...
                        label: Text(
                          _isPublishing ? 'جاري البث...' : 'بث الإعلان الآن 🚀',
          ...
          Row(
            children: [
              ...
              AdminFilterChips(
                options: const ['الكل', 'نشطة فقط 🟢', 'للأطباء 🩺', 'للصيدليات 💊', 'للمرضى 👥', 'عاجلة ⚡'],
  ...
  Widget _buildRoleChip(String label, String value) {
    ...
    return InkWell(
      onTap: () => setState(() => _targetRole = value),
      borderRadius: BorderRadius.circular(10),
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 180),
